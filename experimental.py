...
def find_darkest_cnts(contours, nail_cords, blur, top_grip, bottom_grip, condition_num):
	...
	rect_list = [rect for rect in rect_list if rect[0] > nail_cords[0] - 65 and rect[0] + rect[2] < nail_cords[0] + nail_cords[2] + 65 and rect[1]*rect[3] > 10000]
	rect_list = sorted(rect_list, key = lambda rect:rect[1]+rect[3], reverse = False)
	if len(rect_list) <= 1:
		return (0,0,0,0), (0,0,0,0)
	...
	for i in range(len(rect_list)):
		...
		if lastCombined == True:
			lastCombined = False
			continue

		if rect_list[i][1] - (rect_list[i-1][1] + rect_list[i-1][3]) <= 75 and rect_list[i][1] - (rect_list[i-1][1] + rect_list[i-1][3]) > 0:
			lastCombined = True
			if rect_list[i][0] < rect_list[i-1][0]:
				start = rect_list[i][0]
			else:
				start = rect_list[i-1][0]
			if rect_list[i][0] + rect_list[i][2] > rect_list[i-1][0] + rect_list[i-1][2]:
				width  = (rect_list[i][0] + rect_list[i][2]) - start
			else:
				width = (rect_list[i-1][0] + rect_list[i-1][2]) - start
			#print(f"combined {rect_list[i-1]} up with {rect_list[i]}. NEW RECTANGLE IS: {(start, rect_list[i-1][1], width, (rect_list[i][1] + rect_list[i][3]) - rect_list[i-1][1])}")

			combinedList.append((start, rect_list[i-1][1], width, (rect_list[i][1] + rect_list[i][3]) - rect_list[i-1][1]))
			#cv2.rectangle(tempCopy, (rect_list[i][0], rect_list[i][1]), (rect_list[i][0] + rect_list[i][2], rect_list[i][1] + rect_list[i][3]), (255,0,0), 3)
			#cv2.rectangle(tempCopy, (rect_list[i-1][0], rect_list[i-1][1]), (rect_list[i-1][0] + rect_list[i-1][2], rect_list[i-1][1] + rect_list[i-1][3]), (0,255,0), 3)
			#cv2.rectangle(tempCopy, (start, rect_list[i-1][1]), (start + width, rect_list[i][1] + rect_list[i][3]), (0,0,255), 3)
		else:
			lastCombined = False
			#print(f"didn't combine {rect_list[i]} with {rect_list[i-1]}")
			combinedList.append(rect_list[i])

	cropped_list = [(i, blur[x[1]:x[1]+x[3], x[0]:x[0]+x[2]]) for i, x in enumerate(combinedList)]
	...
	if len(darkest_region) >= 2:
		...
		tmpbenchmark1, tmpbenchmark2 = combinedList[darkest_region[0][0]], combinedList[darkest_region[1][0]]
		permbenchmark1, permbenchmark2 = benchmark_validity(nail_cords, tmpbenchmark1, tmpbenchmark2, top_grip, bottom_grip)
		bl = tempcheck(permbenchmark1, condition_num)
		if bl == False:
			#print("not able to find them by darkest area")
			return (0,0,0,0),(0,0,0,0)
		#print("found them by darkest area")
		return combinedList[darkest_region[0][0]], combinedList[darkest_region[1][0]]
	else:
		return (0,0,0,0), (0,0,0,0)
